...
원래 h3 는 v2 로 미뤄 두었고, 근거는 Nuxt 4 가 아직 h3 v1 을 쓰는데 v2 가 RC 라 곧 두 메이저로
쪼개야 한다는 것이었다. 그 판단을 뒤집었다.

쪼개는 비용이 예상보다 작다는 것이 만들면서 확인됐다. h3 v1 에 이미 `toWebRequest` 가 있어서
어댑터 전체가 코어에 표준 `Request` 를 넘기고 결과를 `appendResponseHeader` 로 되돌리는 일이
전부다. v2 전환은 `framework.ts` 한 파일의 변경이며, 이것이 접근 지점을 한 곳에 모으기로 한
설계의 회수 지점이다. 진짜로 갈라야 하면 §2-A-6 의 alias 서브패키지가 그대로 적용된다.

Next 어댑터보다 단순하다. 쓰기 경로가 하나뿐이라 쿠키 저장소 폴백이 없고, 따라서 동명 쿠키
문제도 속성 유실도 없다. `onUnappliable` 은 받아들이되 무시한다. React 서버 컴포넌트 제약을
설명하는 옵션이고 h3 에는 대응물이 없다. §6 에서 "코어 정책에 두되 Next 어댑터만 해석한다"고
적은 가안이 실제로 그렇게 동작하는 것을 두 어댑터로 확인했다.

테스트에 mock 이 전혀 필요 없다. h3 앱이 평범한 node 리스너라 14개 통합 테스트가 모두 실제
서버를 띄운다. hop-by-hop 테스트는 백엔드가 진짜로 gzip 을 보내게 해서, 헤더를 그대로 넘기면
브라우저가 디코딩에 실패한다는 ADR-0004 의 근거를 끝에서 끝까지 재현한다.
